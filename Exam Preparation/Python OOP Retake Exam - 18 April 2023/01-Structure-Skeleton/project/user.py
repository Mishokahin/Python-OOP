class User:
    RATING_INCREASE_STEP = 0.5
    MAX_RATING = 10
    RATING_DECREASE_STEP = 2.0
    MIN_RATING = 0
    ...
    def increase_rating(self):
        self.rating += self.RATING_INCREASE_STEP
        if self.rating > self.MAX_RATING:
            self.rating = self.MAX_RATING

    def decrease_rating(self):
        if self.rating - self.RATING_DECREASE_STEP < self.MIN_RATING:
            self.rating = self.MIN_RATING
            self.is_blocked = True
        else:
            self.rating -= self.RATING_DECREASE_STEP
